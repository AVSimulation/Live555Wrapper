...
private:
	SubsessionBuffer(const SubsessionBuffer&);
...
public:
	SubsessionIOState(MP4FileSink& sink, MediaSubsession& subsession);
...
public:
	SubsessionIOState(const SubsessionIOState&);
	...
	typedef unsigned (MP4FileSink::*atomCreationFunc)();
...
////////// MP4FileSink implementation //////////

MP4FileSink::MP4FileSink(UsageEnvironment& env,
	...
	fMovieFPS(movieFPS), fMaxTrackDurationM(0)
{
	...
	while ((subsession = iter.next()) != NULL) {
		...
		// we don't overwrite the user defined fps by subsession->videoFPS() to keep the user choice
...
MP4FileSink::~MP4FileSink() {
	...
	while ((subsession = iter.next()) != NULL) {
		...
		SubsessionIOState* ioState = static_cast<SubsessionIOState*>(subsession->miscPtr);
...
MP4FileSink*
MP4FileSink::createNew(UsageEnvironment& env,
MediaSession& inputSession,
char const* outputFileName,
unsigned bufferSize,
unsigned short movieWidth,
...
Boolean generateHintTracks) {
	MP4FileSink* newSink =
		new MP4FileSink(env, inputSession, outputFileName, bufferSize, movieWidth, movieHeight, movieFPS,
...
Boolean MP4FileSink::startPlaying(afterPlayingFunc* afterFunc,
...
Boolean MP4FileSink::continuePlaying() {
...
struct timeval presentationTime,
	...
	SubsessionIOState* ioState = static_cast<SubsessionIOState*>(clientData);
	...
	if (numTruncatedBytes > 0) {
		ioState->envir() << "MP4FileSink::afterGettingFrame(): The input frame data was too large for our buffer.  "
			<< numTruncatedBytes
...
void MP4FileSink::onSourceClosure(void* clientData) {
	SubsessionIOState* ioState = static_cast<SubsessionIOState*>(clientData);
	ioState->onSourceClosure();
}

void MP4FileSink::onSourceClosure1() {
...
void MP4FileSink::onRTCPBye(void* clientData) {
	SubsessionIOState* ioState = static_cast<SubsessionIOState*>(clientData);
...
void MP4FileSink::completeOutputFile() {
...
SubsessionIOState::SubsessionIOState(MP4FileSink& sink,
	MediaSubsession& subsession)
	...
	fQTcomponentSubtype(0), fQTcomponentName(0), 
	fQTMediaInformationAtomCreator(0), fQTMediaDataAtomCreator(0),
	fQTAudioDataType(0), fQTSoundSampleVersion(0), fQTTimeScale(0),
	fQTTimeUnitsPerSample(0), fQTBytesPerFrame(0), fQTSamplesPerFrame(0),
	fQTDurationM(0), fQTDurationT(0), fTKHD_durationPosn(0), fQTInitialOffsetDuration(0)
{
...
Boolean SubsessionIOState::setQTstate() {
	...
	do {
		...
		if (isHintTrack()) {
			...
			fQTMediaInformationAtomCreator = &MP4FileSink::addAtom_gmhd;
			fQTMediaDataAtomCreator = &MP4FileSink::addAtom_rtp;
		}
			...
			fQTMediaInformationAtomCreator = &MP4FileSink::addAtom_smhd;
			fQTMediaDataAtomCreator
				= &MP4FileSink::addAtom_soundMediaGeneral; // by default
			...
			if (strcmp(fOurSubsession.codecName(), "X-QT") == 0 ||
				...
				fQTMediaDataAtomCreator = &MP4FileSink::addAtom_genericMedia;
			}
				...
				fQTMediaDataAtomCreator = &MP4FileSink::addAtom_dummy;
			...
			fQTMediaInformationAtomCreator = &MP4FileSink::addAtom_vmhd;
...
void SubsessionIOState::afterGettingFrame(unsigned packetDataSize, struct timeval presentationTime) {
	...
	else if (fQTMediaDataAtomCreator == &MP4FileSink::addAtom_Qclp) {
	...
	// Frame changed
	Boolean hack264 = strcmp(fOurSubsession.codecName(), "H264") == 0;

	if (!hack264 || (*fBuffer->dataStart() & 0x1F) < 6) // AVSimulation patch, SKIP H264 SEI => problem with Epiphan
	{
		useFrame(*fBuffer);
...
void SubsessionIOState::useFrame(SubsessionBuffer& buffer) {
	...
	Boolean avcHack = fQTMediaDataAtomCreator == &MP4FileSink::addAtom_avc1;
...
Boolean SubsessionIOState::syncOK(struct timeval presentationTime) {
	MP4FileSink& s = fOurSink; // abbreviation
	...
	if (s.fNumSyncedSubsessions < s.fNumSubsessions) {
		...
		if (!fHaveBeenSynced) {
			...
			if (fOurSubsession.rtpSource()->hasBeenSynchronizedUsingRTCP()) {
				// H264 ?
				if (fQTMediaDataAtomCreator == &MP4FileSink::addAtom_avc1) {
...
unsigned MP4FileSink::addWord64(u_int64_t word) {
...
unsigned MP4FileSink::addWord(unsigned word) {
...
unsigned MP4FileSink::addHalfWord(unsigned short halfWord) {
	addByte((unsigned char)(halfWord >> 8)); addByte((unsigned char)halfWord);

	return 2;
}

unsigned MP4FileSink::addZeroWords(unsigned numWords) {
...
unsigned MP4FileSink::add4ByteString(char const* str) {
	addByte(str[0]); addByte(str[1]); addByte(str[2]); addByte(str[3]);

	return 4;
}

unsigned MP4FileSink::addArbitraryString(char const* str,
	...
	if (oneByteLength) {
		...
		if (strLength >= 256) {
			envir() << "MP4FileSink::addArbitraryString(\""
...
unsigned MP4FileSink::addAtomHeader(char const* atomName) {
...
unsigned MP4FileSink::addAtomHeader64(char const* atomName) {
...
void MP4FileSink::setWord(int64_t filePosn, unsigned size) {
	do {
	...
	envir() << "MP4FileSink::setWord(): SeekFile64 failed (err "
		<< envir().getErrno() << ")\n";
}

void MP4FileSink::setWord64(int64_t filePosn, u_int64_t size) {
	do {
		if (SeekFile64(fOutFid, filePosn, SEEK_SET) < 0) break;
		addWord64(size);
		if (SeekFile64(fOutFid, 0, SEEK_END) < 0) break; // go back to where we were
	...
	// One of the SeekFile64()s failed, probable because we're not a seekable file
	envir() << "MP4FileSink::setWord64(): SeekFile64 failed (err "
		<< envir().getErrno() << ")\n";
}
...
#define addAtom(name) \
    unsigned MP4FileSink::addAtom_##name() { \
...
MediaSubsession* subsession;
while ((subsession = iter.next()) != NULL) {
	fCurrentIOState = static_cast<SubsessionIOState*>(subsession->miscPtr);
...
unsigned MP4FileSink::addAtom_hdlr2() {
	int64_t initFilePosn = TellFile64(fOutFid);
	...
	unsigned MP4FileSink::addAtom_genericMedia() {
		int64_t initFilePosn = TellFile64(fOutFid);
		...
		unsigned MP4FileSink::addAtom_soundMediaGeneral() {
			int64_t initFilePosn = TellFile64(fOutFid);
			...
			unsigned MP4FileSink::addAtom_Qclp() {
				...
				unsigned MP4FileSink::addAtom_mp4a() {
					unsigned size = 0;
					// The beginning of this atom looks just like a general Sound Media atom,
					// except with a version field of 1:
					int64_t initFilePosn = TellFile64(fOutFid);
					...
					unsigned MP4FileSink::addAtom_rtp() {
						int64_t initFilePosn = TellFile64(fOutFid);
						...
						unsigned MP4FileSink::addAtom_sdp() {
							int64_t initFilePosn = TellFile64(fOutFid);
							...
							for (p1 = sdpLines; *p1 != '\0'; ++p1) {
								...
								if (*p3 == '\0') {
									...
									sprintf(&newSDPLines[i], "%u", fCurrentIOState->fTrackID);
									i = afterTrackNumPosn;
									int j = i + (int)strlen(&newSDPLines[i]);
									while (1) {
							...
							if (rtpSource != NULL) {
								...
								unsigned int rtpmapStringLength = (unsigned int) strlen(ourSubsession.codecName()) + 20;
							...
							unsigned MP4FileSink::addAtom_dummy() {
								int64_t initFilePosn = TellFile64(fOutFid);
